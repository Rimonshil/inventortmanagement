  ...
  <select class="form-control" id="exampleFormControlSelect1" v-model="form.salary_month">
   ...
   <small class="text-danger" v-if="errors.salary_month"> {{ errors.salary_month[0] }} </small>   
     ...
     <div class="col-md-6">
         ...
         <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Enter Your Sallery" v-model="form.sallary">
         <small class="text-danger" v-if="errors.sallery"> {{ errors.sallary[0] }} </small>
            </div>     
...
<script type="text/javascript">
  ...
  methods:{
   SalaryPaid() {
   let id = this.$route.params.id;
            axios.get('/api/salary/paid'+id,this.form)
            .then(res => {
                 Toast.fire({
                 icon: 'success',
